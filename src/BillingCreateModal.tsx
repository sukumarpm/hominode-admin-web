...
export function BillingCreateModal({ s, onClose }: { s: Session; onClose: () => void }) {
  const now = new Date();
  ...
  const residentFlatIds = new Set(
    residents.rows.map((row) => str(row.data.flatId)).filter(Boolean),
  );

  const unitsForBuilding = units.rows
    .filter((row) => str(row.data.buildingId) === buildingId && residentFlatIds.has(row.id))
    .map((row) => ({
      id: row.id,
      label:
  ...
  async function submit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setResult(
        `${response.created} bill${response.created === 1 ? '' : 's'} created successfully.${
          response.skipped
            ? ` ${response.skipped} resident record${response.skipped === 1 ? '' : 's'} skipped.`
            : ''
        }`,
      );
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Maintenance bills could not be created.');
    } finally {
      setBusy(false);
    }
  }
          ...
          {scope === 'unit' && (
            <label>
              ...
              <select
                value={flatId}
                required
                disabled={!buildingId || units.loading || residents.loading}
